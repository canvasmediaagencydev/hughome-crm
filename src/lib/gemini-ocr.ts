...
async function processReceiptInternal(
  base64: string,
  mimeType: string
): Promise<OCRResult> {
  const apiKey =
...
export async function processReceiptWithGemini(
  ...
  try {
    ...
    // Get MIME type
    const mimeType =
      imageFile.type || mime.getType(imageFile.name) || "image/jpeg";
...
export function convertThaiDateToISO(thaiDate: string): string | null {
  try {
    ...
    if (simpleMatch) {
      ...
      const gregorianYear =
        parseInt(year) > 2400 ? parseInt(year) - 543 : parseInt(year);

      // Create ISO date string (YYYY-MM-DD)
      const isoDate = `${gregorianYear}-${month.padStart(
        2,
        "0"
      )}-${day.padStart(2, "0")}`;
