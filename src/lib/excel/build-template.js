...
/**
 * @param {{code: string, full_name: string}[]} reps พนักงานขายที่ active — ต้องมีอย่างน้อย 1 คน
 * @returns {import('exceljs').Workbook}
 */
function buildSalesTemplate(reps) {
  if (!Array.isArray(reps) || reps.length === 0) {
    throw new Error('ต้องมีพนักงานขายอย่างน้อย 1 คน — เพิ่มรายชื่อที่ /admin/sales-reps ก่อน')
  }
  const seen = new Set()
  for (const r of reps) {
    if (!r || !r.code || !r.full_name) throw new Error('พนักงานขายต้องมีทั้ง code และ full_name')
    const k = String(r.code).toUpperCase()
    if (seen.has(k)) throw new Error(`รหัสพนักงานซ้ำ: ${r.code}`)
    seen.add(k)
    if (String(r.code).includes(SPEC.salesRepSeparator)) {
      throw new Error(`รหัสพนักงาน "${r.code}" มีตัวคั่น "${SPEC.salesRepSeparator}" — parser จะตัดผิด`)
    }
  }
  ...
  // ---------- แผ่นซ่อนเก็บรายชื่อพนักงาน (แหล่งข้อมูลของ dropdown) ----------
  ...
  for (let r = SPEC.headerRow + 1; r <= lastRow; r++) {
    ws.getCell(`${repCol}${r}`).dataValidation = {
      ...
      errorStyle: 'stop',
      errorTitle: 'พนักงานขายไม่ถูกต้อง',
  ...
  const guideRows = [
    ...
    [
      ...
      req('purchase_date'),
      'วันที่',
      'วันที่เกิดรายการซื้อ · ใช้กำหนดเดือนที่ได้แต้มและวันหมดอายุแต้ม · ต้องอยู่ในสัปดาห์ที่บัญชีเลือกตอนอัปโหลด · กรอก พ.ศ. ได้ ระบบแปลงให้',
    ],
    [
      col('bill_no').header,
      req('bill_no'),
    ...
    [col('customer_name').header, req('customer_name'), 'ข้อความ', 'ใช้ตรวจทานเฉยๆ · ระบบจับคู่ด้วยเบอร์โทรอย่างเดียว'],
    ...
    ['── ตัวอย่างการกรอก ──'],
    HEADERS,
    ['01/07/2026', 'INV-2026-0731', '0812345678', 'สมชาย ใจดี', 2500, 0, repLabel(reps[0]), ''],
    [
      '03/07/2026',
      'INV-2026-0742',
      '0898765432',
    ...
    [],
    ['── แคมเปญแต้มพิเศษ ──'],
    ['พนักงานขาย "ไม่ต้อง" กรอกโค้ดโปรโมชันในไฟล์นี้ และไม่มีคอลัมน์ให้กรอก'],
    ...
    ['5. เลขที่บิลซ้ำกับที่ส่งไปแล้ว', 'ระบบ reject แถวนั้น · ถ้าบิลเดิมผิดต้องให้บัญชียกเลิก batch เดิมก่อน'],
    ['6. วันที่ซื้อหลุดออกนอกสัปดาห์ที่ส่ง', 'ระบบ reject แถวนั้น · ยอดข้ามสัปดาห์ให้แยกไฟล์'],
    ['7. พิมพ์ชื่อพนักงานขายเอง', 'ต้องเลือกจาก dropdown เท่านั้น'],
    ['8. มีแถวว่างคั่นกลาง', 'ระบบข้ามให้ แต่ควรลบออกเพื่อความชัดเจน'],
    [],
    ['── ขั้นตอนการทำงาน ──'],
    [`1. พนักงานขายกรอกลงแผ่น "${SPEC.sheetName}" ทุกวัน (paste หลายรายการพร้อมกันได้)`],
    ['2. ส่งไฟล์ให้บัญชีทุกสัปดาห์'],
    ['3. บัญชี upload เข้าระบบหลังบ้าน → เลือกช่วงสัปดาห์ → ตรวจหน้า preview → กดยืนยัน'],
    ['4. แต้มเข้าบัญชีลูกค้าทันที และลูกค้าได้รับแจ้งทาง LINE'],
    ['5. ผู้จัดการสุ่มตรวจจากรายงานรายสัปดาห์ย้อนหลัง (เทียบเลขที่บิล + ชื่อพนักงานขาย)'],
    [],
    [
