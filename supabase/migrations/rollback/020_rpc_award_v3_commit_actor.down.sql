-- rollback 020_rpc_award_v3_commit_actor.sql
-- คืน award_points_from_batch เป็นเวอร์ชัน v2 ของ 017 (1 argument ไม่บันทึกคน commit)
-- คัดลอกมาทั้งตัวโดยเจตนา — ไฟล์ rollback ต้องรันได้เองไม่พึ่งไฟล์อื่น
--
-- ⚠️ ถอย 020 แล้วต้องถอย 019 ต่อทันที
--    v2 เซ็ต committed_at โดยไม่เซ็ต committed_by → ชน CHECK point_batches_commit_actor ของ 019
--    ค้างไว้ครึ่งทางคือ commit batch ไม่ได้เลย

DROP FUNCTION IF EXISTS award_points_from_batch(uuid, uuid);
